...
{
	//攻撃アニメーション
	attack_image.Load("res/effect/sword/0.png");
	attack_anime.LoadHandles(attack_image.GetGraphicsHandl());

	attack_image.Load("res/effect/sword/1.png");
	attack_anime.LoadHandles(attack_image.GetGraphicsHandl());

	attack_image.Load("res/effect/sword/2.png");
	attack_anime.LoadHandles(attack_image.GetGraphicsHandl());

	attack_image.Load("res/effect/sword/3.png");
	attack_anime.LoadHandles(attack_image.GetGraphicsHandl());

	attack_image.Load("res/effect/sword/4.png");
	attack_anime.LoadHandles(attack_image.GetGraphicsHandl());

	attack_image.Load("res/effect/sword/5.png");
	attack_anime.LoadHandles(attack_image.GetGraphicsHandl());

	//防御アニメーション
	defense_image.Load("res/effect/defense/0.png");
	defense_anime.LoadHandles(defense_image.GetGraphicsHandl());

	defense_image.Load("res/effect/defense/1.png");
	defense_anime.LoadHandles(defense_image.GetGraphicsHandl());

	defense_image.Load("res/effect/defense/2.png");
	defense_anime.LoadHandles(defense_image.GetGraphicsHandl());

	defense_image.Load("res/effect/defense/3.png");
	defense_anime.LoadHandles(defense_image.GetGraphicsHandl());

	defense_image.Load("res/effect/defense/4.png");
	defense_anime.LoadHandles(defense_image.GetGraphicsHandl());

	defense_image.Load("res/effect/defense/5.png");
	defense_anime.LoadHandles(defense_image.GetGraphicsHandl());

	defense_image.Load("res/effect/defense/6.png");
	defense_anime.LoadHandles(defense_image.GetGraphicsHandl());
...
void Player::Updata()
{
	...
	if (input_left.GetPressMouse(MOUSE_INPUT_LEFT) == true)
	{
		...
		if (is_attack == false)
		{
			...
			attack_rote = std::atan2(input_left.GetMousePosition().y - player_pos.y, input_left.GetMousePosition().x - player_pos.x);

			//極座標の計算
			attack_pos.x = (attack_range * std::cos(attack_rote)) + player_pos.x;
			attack_pos.y = (attack_range * std::sin(attack_rote)) + player_pos.y;
