...
{	
	...
	siren.SwitchButtonBOX_Load("texter/command/siren.png", 128, 64);//試練ボタンの画像をロード
	seisaku.SwitchButtonBOX_Load("texter/command/seisaku.png", 128, 64);//政策ボタンの画像をロード
	zeiritu.SwitchButtonBOX_Load("texter/command/zeiritu.png", 128, 64);//税率ボタンの画像をロード
	per_0.SwitchButtonBOX_Load("texter/command/0_per.png", 128, 64);//0%ボタンの画像を
	per_5.SwitchButtonBOX_Load("texter/command/5_per.png", 128, 64);//5%ボタンの画像をロード
	per_10.SwitchButtonBOX_Load("texter/command/10_per.png", 128, 64);//10%ボタンの画像をロード
	per_15.SwitchButtonBOX_Load("texter/command/15_per.png", 128, 64);//15%ボタンの画像をロード
	per_20.SwitchButtonBOX_Load("texter/command/20_per.png", 128, 64);//20%ボタンの画像をロード
...
{
	jinkou = musyoku + nougyousya + sanngyousya + syougyousya;//総人口計算
	...
	siren.SwitchButtonBOX_Init(VGet(ku_position.x, ku_position.y, 0), VGet(128, 64, 0), "siren");//試練ボタンの初期化
	seisaku.SwitchButtonBOX_Init(VGet(ku_position.x, ku_position.y + 64, 0), VGet(128, 64, 0), "seisaku");//政策ボタンの初期化
	zeiritu.SwitchButtonBOX_Init(VGet(ku_position.x + 128, ku_position.y, 0), VGet(128, 64, 0), "zeiritu");//税率ボタンの初期化
	per_0.SwitchButtonBOX_Init(VGet(ku_position.x + (128 * 2), ku_position.y, 0), VGet(128, 64, 0), "per_0");//0%ボタンの初期化
	per_5.SwitchButtonBOX_Init(VGet(ku_position.x + (128 * 2), ku_position.y + 64, 0), VGet(128, 64, 0), "per_5");//5%ボタンの初期化
	per_10.SwitchButtonBOX_Init(VGet(ku_position.x + (128 * 2), ku_position.y + (64 * 2), 0), VGet(128, 64, 0), "per_10");//10%ボタンの初期化
	per_15.SwitchButtonBOX_Init(VGet(ku_position.x + (128 * 2), ku_position.y + (64 * 3), 0), VGet(128, 64, 0), "per_15");//15%ボタンの初期化
	per_20.SwitchButtonBOX_Init(VGet(ku_position.x + (128 * 2), ku_position.y + (64 * 4), 0), VGet(128, 64, 0), "per_20");//20%ボタンの初期化
	...
	for (int count = 0; count < 9; ++count)
	{
		...
		if (field[count].SwitchButtonBOX_Update(MOUSE_INPUT_LEFT, diploidEngineImpact, diploidEngineInput) == true)
		{
			...
			for (int flag_count = 0; flag_count < 9; ++flag_count)
			{
				if (count != flag_count)
				{
					if (field[flag_count].GetSwitchButton_Flag() == true)
					{
						...
						siren.SwitchButton_Flag_Change(false);//試練ボタンをFALSEに
						seisaku.SwitchButton_Flag_Change(false);//政策ボタンをFALSEに
						zeiritu.SwitchButton_Flag_Change(false);//税率ボタンをFALSEに
						per_0.SwitchButton_Flag_Change(false);//0%ボタンをFALSEに
						per_5.SwitchButton_Flag_Change(false);//5%ボタンをFALSEに
						per_10.SwitchButton_Flag_Change(false);//10%ボタンをFALSEに
						per_15.SwitchButton_Flag_Change(false);//15%ボタンをFALSEに
						per_20.SwitchButton_Flag_Change(false);//20%ボタンをFALSEに
			...
			if (count == 4)
			{
				...
				siren_system.OnePushBOX_Impact(diploidEngineImpact, siren.box);//試練ボタンの追加
				seisaku_system.OnePushBOX_Impact(diploidEngineImpact, seisaku.box);//政策ボタンの追加

				seisaku_system.FlagReset_OneDeleteBOX();//政策ボタンの削除フラグをリセット			
				siren_system.FlagReset_OneDeleteBOX();//試練ボタンの削除フラグをリセット			


				//試練ボタンがオンのとき
				if (siren.SwitchButtonBOX_Update(MOUSE_INPUT_LEFT, diploidEngineImpact, diploidEngineInput) == true)
				{
					seisaku.SwitchButton_Flag_Change(false);//政策ボタンをFALSEに

					zeiritu.SwitchButton_Flag_Change(false);//税率ボタンをFALSEに
					zeiritu_system.OneDeleteBOX_Impact(diploidEngineImpact, "zeiritu");//税率ボタンの削除
					zeiritu_system.FlagReset_OnePushBOX();//税率ボタンの追加フラグをリセット

				}

				//政策ボタンがオンのとき
				if (seisaku.SwitchButtonBOX_Update(MOUSE_INPUT_LEFT, diploidEngineImpact, diploidEngineInput) == true)
				{
					siren.SwitchButton_Flag_Change(false);//試練ボタンをFALSEに	

					zeiritu_system.OnePushBOX_Impact(diploidEngineImpact, zeiritu.box);//税率ボタンの追加
					zeiritu_system.FlagReset_OneDeleteBOX();//税率ボタンの削除フラグをリセット
															
					//税率ボタンがオンのとき
					if (zeiritu.SwitchButtonBOX_Update(MOUSE_INPUT_LEFT, diploidEngineImpact, diploidEngineInput) == true)
					{
						per_0_system.OnePushBOX_Impact(diploidEngineImpact, per_0.box);//0%ボタンの追加
						per_5_system.OnePushBOX_Impact(diploidEngineImpact, per_5.box);//5%ボタンの追加
						per_10_system.OnePushBOX_Impact(diploidEngineImpact, per_10.box);//10%ボタンの追加
						per_15_system.OnePushBOX_Impact(diploidEngineImpact, per_15.box);//15%ボタンの追加
						per_20_system.OnePushBOX_Impact(diploidEngineImpact, per_20.box);//20%ボタンの追加

						per_0_system.FlagReset_OneDeleteBOX();//0%ボタンの削除フラグをリセット
						per_5_system.FlagReset_OneDeleteBOX();//5%ボタンの削除フラグをリセット
						per_10_system.FlagReset_OneDeleteBOX();//10%ボタンの削除フラグをリセット
						per_15_system.FlagReset_OneDeleteBOX();//15%ボタンの削除フラグをリセット
						per_20_system.FlagReset_OneDeleteBOX();//20%ボタンの削除フラグをリセット

						//0%ボタンがオンのとき
						if (per_0.SwitchButtonBOX_Update(MOUSE_INPUT_LEFT, diploidEngineImpact, diploidEngineInput) == true)
						{
							per_5.SwitchButton_Flag_Change(false);//5%ボタンをオフにする。
							per_10.SwitchButton_Flag_Change(false);//10%ボタンをオフにする。
							per_15.SwitchButton_Flag_Change(false);//15%ボタンをオフにする。
							per_20.SwitchButton_Flag_Change(false);//20%ボタンをオフにする。

							zeiritu_scale = 0.0f;
						}

						//5%ボタンがオンのとき
						if (per_5.SwitchButtonBOX_Update(MOUSE_INPUT_LEFT, diploidEngineImpact, diploidEngineInput) == true)
						{
							per_0.SwitchButton_Flag_Change(false);//0%ボタンをオフにする。
							per_10.SwitchButton_Flag_Change(false);//10%ボタンをオフにする。
							per_15.SwitchButton_Flag_Change(false);//15%ボタンをオフにする。
							per_20.SwitchButton_Flag_Change(false);//20%ボタンをオフにする。
						...
						//10%ボタンがオンのとき
						if (per_10.SwitchButtonBOX_Update(MOUSE_INPUT_LEFT, diploidEngineImpact, diploidEngineInput) == true)
						{
							per_0.SwitchButton_Flag_Change(false);//0%ボタンをオフにする。
							per_5.SwitchButton_Flag_Change(false);//5%ボタンをオフにする。
							per_15.SwitchButton_Flag_Change(false);//15%ボタンをオフにする。
						...
						//15%ボタンがオンのとき
						if (per_15.SwitchButtonBOX_Update(MOUSE_INPUT_LEFT, diploidEngineImpact, diploidEngineInput) == true)
						{
							per_0.SwitchButton_Flag_Change(false);//0%ボタンをオフにする。
							per_5.SwitchButton_Flag_Change(false);//5%ボタンをオフにする。
							per_10.SwitchButton_Flag_Change(false);//10%ボタンをオフにする。
						...
						//20%ボタンがオンのとき
						if (per_20.SwitchButtonBOX_Update(MOUSE_INPUT_LEFT, diploidEngineImpact, diploidEngineInput) == true)
						{
							per_0.SwitchButton_Flag_Change(false);//0%ボタンをオフにする。
							per_5.SwitchButton_Flag_Change(false);//5%ボタンをオフにする。
							per_10.SwitchButton_Flag_Change(false);//10%ボタンをオフにする。
					...
					{		
						...
						per_0_system.OneDeleteBOX_Impact(diploidEngineImpact, "per_0");//0%ボタンの削除
						per_5_system.OneDeleteBOX_Impact(diploidEngineImpact, "per_5");//5%ボタンの削除
						per_10_system.OneDeleteBOX_Impact(diploidEngineImpact, "per_10");//10%ボタンの削除
						per_15_system.OneDeleteBOX_Impact(diploidEngineImpact, "per_15");//15%ボタンの削除
						per_20_system.OneDeleteBOX_Impact(diploidEngineImpact, "per_20");//20%ボタンの削除

						per_0_system.FlagReset_OnePushBOX();//0%ボタンの追加フラグをリセット
						per_5_system.FlagReset_OnePushBOX();//5%ボタンの追加フラグをリセット
						per_10_system.FlagReset_OnePushBOX();//10%ボタンの追加フラグをリセット
						per_15_system.FlagReset_OnePushBOX();//15%ボタンの追加フラグをリセット
						per_20_system.FlagReset_OnePushBOX();//20%ボタンの追加フラグをリセット
			...
			else
			{
				...
				siren_system.OneDeleteBOX_Impact(diploidEngineImpact, "siren");//試練ボタンの削除
				seisaku_system.OneDeleteBOX_Impact(diploidEngineImpact, "seisaku");//政策ボタンの削除
				zeiritu_system.OneDeleteBOX_Impact(diploidEngineImpact, "zeiritu");//税率ボタンの削除
				per_0_system.OneDeleteBOX_Impact(diploidEngineImpact, "per_0");//0%ボタンの削除
				per_5_system.OneDeleteBOX_Impact(diploidEngineImpact, "per_5");//5%ボタンの削除
				per_10_system.OneDeleteBOX_Impact(diploidEngineImpact, "per_10");//10%ボタンの削除
				...
				seisaku_system.FlagReset_OnePushBOX();//政策ボタンの追加フラグをリセット
				siren_system.FlagReset_OnePushBOX();//試練ボタンの追加フラグをリセット
				zeiritu_system.FlagReset_OnePushBOX();//税率ボタンの追加フラグをリセット	
				per_0_system.FlagReset_OnePushBOX();//0%ボタンの追加フラグをリセット
				per_5_system.FlagReset_OnePushBOX();//5%ボタンの追加フラグをリセット
				per_10_system.FlagReset_OnePushBOX();//10%ボタンの追加フラグをリセット
...
void DiploidEngineApp::Draw()//結果を描写する処理
{
	...
	for (int count = 0; count < 9; ++count)
	{
		...
		if (field[count].GetSwitchButton_Flag() == true)
		{
			...
			if (count == 4)
			{
				siren.SwitchButtonBOX_Draw();//試練ボタンの描画
				seisaku.SwitchButtonBOX_Draw();//政策ボタンの描画

				//政策ボタンが押されていたら
				if (seisaku.GetSwitchButton_Flag() == true)
				{
					zeiritu.SwitchButtonBOX_Draw();//税率ボタンを描画

					//税率ボタンが押されていたら
					if (zeiritu.GetSwitchButton_Flag() == true)
					{
						per_0.SwitchButtonBOX_Draw();//0%ボタンの描画
						per_5.SwitchButtonBOX_Draw();//5%ボタンの描画
						per_10.SwitchButtonBOX_Draw();//10%ボタンの描画
						per_15.SwitchButtonBOX_Draw();//15%ボタンの描画
						per_20.SwitchButtonBOX_Draw();//20%ボタンの描画
