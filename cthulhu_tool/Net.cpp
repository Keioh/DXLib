...
void NET_GAMING::NetWork_wait(int host_flag, FILER_S host_info)
{
	while (ProcessMessage() == 0 && ScreenFlip() == 0 && ClearDrawScreen() == 0 && break_flag == true)
	{
		...
		if (animation >= 255) animation = 0;//アニメーション変数が255以上になったら0にする
...
{
	NetWorkSend(host_handl, data, 2);	//Hostにデータを送る
}
...
void NET_GAMING::NetSend(int* data, FILER_S host_info)//ホスト側から送る処理
{
	...
	for (int i = 0; i < host_info.member; i++)
	{
		NetWorkSend(net[i], data, 2);	//他のPCにデータを送る
	}
}
...
int NET_GAMING::NetReadHost(int* data, FILER_S host_info)//ホスト側の読み込み処理
{
	...
	for (int i = 0; i < host_info.member; i++)
	{
		if (GetNetWorkDataLength(net[i]) >= 0) return NetWorkRecv(net[i], data, 2);//GetNetWorkDataLength(net[i]));//受信データからintバッファーにデータを移す。
	}
}

int NET_GAMING::NetRead(int* data)//ホストからの読み込み処理
{
	if (GetNetWorkDataLength(host_handl) >= 0) return NetWorkRecv(host_handl, data, 2);//GetNetWorkDataLength(host_handl));//受信データからintバッファーにデータを移す。
}