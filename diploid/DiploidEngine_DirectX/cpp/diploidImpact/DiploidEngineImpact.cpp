...
void DiploidEngineImpact::Updata()
{
	...
	std::thread thread_box_box([this]() { this->ImpactBoxBox(); });//四角と四角の衝突計算の並列処理の実行
	std::thread thread_box_circle([this]() { this->ImpactBoxCircle(); });//四角と円の衝突計算の並列処理の実行
	std::thread thread_point_line([this]() { this->ImpactPointLine(); });//点と線分の衝突計算の並列処理の実行
	std::thread thread_circle_line([this]() { this->ImpactCircleLine(); });//円と線分の衝突計算の並列処理の実行
	std::thread thread_line_line([this]() { this->ImpactLineLine(); });//線分と線分の衝突計算の並列処理の実行
	std::thread thread_line_box([this]() { this->ImpactLineBox(); });//線分と四角の衝突計算の並列処理の実行

	thread_circle_point.join();//円と点の衝突計算を待つ
	thread_box_point.join();//四角と点の衝突計算を待つ
	thread_circle_circle.join();//円と円の衝突計算を待つ
	thread_box_box.join();//四角と四角の衝突計算を待つ
	thread_box_circle.join();//四角と円の衝突計算を待つ
	thread_point_line.join();//点と線分の衝突計算を待つ
	thread_circle_line.join();//円と線分の衝突計算を待つ
